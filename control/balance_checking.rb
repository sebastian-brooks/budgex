...
def get_balance(user, type = 0)
    ...
    when 0
        table = TTY::Table.new(
            ["  BALANCE  ".bright],
            [[" #{balance[:amount].to_f}".color(:crimson)]]
        )
    when 1
        table = TTY::Table.new(
            ["  Balance as of #{Date.parse(date).strftime("%b %d %Y")}  ".bright],
            [[" #{balance[:amount].to_f}".color(:crimson)]]
        )
    end
...
def sub_zero_balance_check(user)
    current_bal = get_balance(user)[1]
    ...
    if scary_dates.size == 0
        puts "You should have enough funds for all your scheduled expenses :)".color(:lightgreen).bright
        puts "Keep up the great work!".color(:lightgreen).bright
    else
        puts "OH SHIT!".color(:crimson).bright.underline.blink
        puts "\nYOU MAY NOT HAVE ENOUGH MONEY ON #{Date.parse(scary_dates.sort[0]).strftime("%b %d %Y")}!".color(:crimson).bright
    end

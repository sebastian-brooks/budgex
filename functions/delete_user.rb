...
def delete_user_process(user)
    ...
    puts "\nWARNING - all of your data will be wiped. There won't be anything to remember you by".red.bright
    attempts = 0
    result = nil
    ...
    if attempts > 3
        puts "\nToo many attempts - get out of here Edward Snowden".color(:indigo).bright
        sleep(2)
    end
    if ! result.nil?
        clear_screen_print_logo()
        puts "Here we go. One last chance to stop us from wiping you off the face of the earth...".color(:cyan)
        sleep(2)
        choices = ["YES", "NO"]
